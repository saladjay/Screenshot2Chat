...
def assign_speaker_by_edges(
    ...
    frequent_xmin_bins = {
        bin_id for bin_id, count in xmin_bins.items()
        if count >= min_count and count / total >= min_ratio
    }
    frequent_xmax_bins = {
        bin_id for bin_id, count in xmax_bins.items()
        if count >= min_count and count / total >= min_ratio
    }
    ...
    left_bins = {
        bin_id for bin_id in frequent_xmin_bins
        if bin_to_value(bin_id) <= left_edge_max_ratio
    }
    right_bins = {
        bin_id for bin_id in frequent_xmax_bins
...
def main():
    ...
    for file_name in os.listdir(input_dir):
        ...
        if 'discord' in file_name.lower() and ratios:
