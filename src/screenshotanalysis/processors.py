...
class ChatMessageProcessor:
    ...
    def format_conversation(self, layout_det_results, text_det_results, padding, image_sizes, ratios=None, app_type=None, log_file=None, use_adaptive=False, screen_width=None):
        ...
        # 新方法：使用自适应检测器
        if use_adaptive and screen_width is not None:
            if log_file:
                print("Using adaptive layout detection method", file=log_file)
            
            # 1. 先为layout_det的框分配说话者
            layout_det_with_speakers = self.assign_speakers_to_layout_det_boxes(
                layout_det_text_boxes, 
                screen_width=screen_width,
                log_file=log_file
            )
            
            # 2. 使用layout_det结果筛选text_det框，并继承说话者信息
            filtered_text_boxes = self.filter_text_boxes_by_layout_det(
                text_det_text_boxes,
            ...
            return self.sort_boxes_by_y(filtered_text_boxes), original_text_boxes
