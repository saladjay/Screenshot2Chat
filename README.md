...
- **🧠 智能学习**: 跨截图学习和记忆说话者的几何特征，保持身份一致�?
- **📊 多种布局**: 支持单列、双列、左对齐、右对齐等多种聊天布局
- **⏱️ 时序验证**: 利用对话交替模式提高说话者识别准确�?
- **💾 持久�?*: 支持记忆数据持久化，跨会话保持学习成�?
- **🔄 自适应**: 自动fallback机制确保在数据不足时的稳定�?
...
detector = ChatLayoutDetector(
    screen_width=720,
    memory_path="chat_memory.json"  # 可选：持久化记�?
)
...
print(f"说话者A: {len(result['A'])} 条消�?)
print(f"说话者B: {len(result['B'])} 条消�?)
print(f"置信�? {result['metadata']['confidence']}")
```
...
| `memory_alpha` | float | 0.7 | 记忆更新的滑动平均系�?|
| `memory_path` | str | None | 记忆持久化路�?|
...
1. **几何分析�?*: KMeans聚类、布局分类、Fallback机制
2. **记忆学习�?*: 跨截图记忆、滑动平均更新、持久化存储
3. **匹配决策�?*: 最小代价匹配、时序一致性验证、置信度计算

## 与旧系统对比

| 特�?| 旧系统（YAML配置�?| 新系统（自适应�?|
|------|-------------------|-----------------|
| 应用类型依赖 | �?需要指�?| �?完全无关 |
| 配置文件 | �?需要YAML | �?无需配置 |
| 跨截图学�?| �?不支�?| �?自动学习 |
...
4. 检查`confidence`字段，低置信度时可能需要人工验�?
...
### 当前文档
- [用户指南](docs/USER_GUIDE.md) - 完整的使用指�?
...
### 开发历�?
- [开发文档归档](history/development/README.md) - 重构过程中的所有开发文�?
...
ScreenshotAnalysis Team @ [COMPANY]
